...
import { NOTE_CATS } from '@/lib/constants';
import { uid } from '@/lib/utils';

export default function W1Notes({w1,setW1}){
  const {notes,events}=w1;
  const setNotes=fn=>setW1(d=>({...d,notes:typeof fn==="function"?fn(d.notes):fn}));
  ...
  const add=()=>{if(!newTit.trim())return;const n={id:uid(),title:newTit,content:"",category:"Geral",eventId:"",createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};setNotes(p=>[n,...p]);setNewTit("");setAddM(false);setActive(n.id);toast("Nota criada");};
  ...
  const linkedEvent=an&&an.eventId?(events||[]).find(e=>e.id===an.eventId):null;
  const catColor={"Roteiro":"#c0392b","Pesquisa":"#1a5276","Entrevista":"#8e44ad","Referência":"#d68910","Geral":"#888"};
              ...
              <div key={n.id} onClick={()=>{setActive(n.id);setAddM(false);}} style={{padding:"9px 14px",cursor:"pointer",background:active===n.id?"#fff":"transparent",borderLeft:active===n.id?"2px solid #1a1a1a":"2px solid transparent",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div style={{minWidth:0,flex:1}}>
                  ...
                  <div style={{display:"flex",gap:4,marginTop:2,alignItems:"center"}}>
                    ...
                    {n.eventId&&<span style={{fontSize:10,color:"#1a5276"}}>◫</span>}
        ...
        {an?(
          <div style={{flex:1,padding:"20px 24px",display:"flex",flexDirection:"column",gap:10}}>
            ...
            <div style={{display:"flex",gap:10,alignItems:"center"}}>
              ...
              <Sel value={an.eventId||""} onChange={e=>upd(an.id,"eventId",e.target.value)} style={{fontSize:11,padding:"3px 8px",flex:1}}>
                <option value="">— Vincular a evento —</option>
                {(events||[]).map(ev=><option key={ev.id} value={ev.id}>{ev.title} ({ev.date})</option>)}
              </Sel>
            </div>
            {linkedEvent&&<div style={{margin:"4px 0",padding:"6px 10px",background:"#eaf2fb",borderRadius:6,display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:12,color:"#1a5276"}}>◫</span><span style={{fontSize:12,color:"#1a5276",fontWeight:600}}>Vinculada a: {linkedEvent.title}</span><span style={{fontSize:11,color:"#6a8faf"}}>({linkedEvent.date})</span></div>}
